...
assert.match(html, /id="focusSearchDialog"[^>]*class="modal focus-search-modal"/);
assert.match(html, /id="focusSearchInput"[^>]*role="combobox"/s);
assert.match(html, /id="focusSearchResults"[^>]*role="listbox"/);
...
assert.match(app, /els\.railSearchBtn\.addEventListener\("click", openFocusSearchDialog\)/);
assert.match(app, /function findSavedShortcuts\(/);
assert.match(app, /function savedShortcutMatchRank\(/);
assert.match(app, /await recordShortcutUse\(shortcut\.id\)/);
assert.match(app, /window\.location\.href = url/);
assert.doesNotMatch(app, /focusSearchInput[\s\S]{0,500}addSearchHistory/);
assert.match(baseStyles, /\.topbar\s*\{[^}]*z-index:\s*50/s);
assert.match(baseStyles, /\.search-history-panel\s*\{[^}]*z-index:\s*200/s);
assert.match(baseStyles, /\.focus-search-modal\s*\{/);
assert.match(baseStyles, /\.focus-search-results\s*\{[^}]*overflow-y:\s*auto/s);
assert.match(baseStyles, /\.focus-search-result \.shortcut-icon\s*\{[^}]*border-radius:\s*var\(--icon-radius\)/s);
